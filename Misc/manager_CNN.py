...
class Manager():
    def __init__(self, file_pth='experiments/misc',
        ...
        if data_partition:
            ...
            test_set               = signal_dataset.SignalDataset('data_final/multigrasp_test', multigrasp=multigrasp, filtering=filtering, cropping=cropping, normalise=False, augment=False)
    ...
    def run_testing(self, test_data):
        ...
        if self.dual_cls:
            ...
            utils.confusion_plotter(cm, cm_filename, False)

            with open(f'{self.file_pth}/test_reports.txt', 'w', encoding="utf-8") as file:
                file.write(report)                                                                  # type: ignore

        # accuracy_table = utils.compute_pairwise_accuracy(
        #     all_mat_targets, all_mat_preds,
        #     all_tex_targets, all_tex_targets,
        #     self.full_dataset.mat_classes,
        #     self.full_dataset.tex_classes
        # )

        # plot_path = f"{self.file_pth}/joint_accuracy_table.png"
        # utils.plot_joint_accuracy_table(
        #     accuracy_table,
        #     self.full_dataset.mat_classes,
        #     self.full_dataset.tex_classes,
        #     plot_path
...
if __name__ == '__main__':
    ...
    manager = Manager(file_pth='experiments/CNN/resnet_CNN',
