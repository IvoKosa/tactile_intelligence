...
class Tactile_CNN(nn.Module):
    def __init__(
        ...
        mat_classes=6,
        tex_classes=6,
        latent_dim=64,        # channels after the last conv in the encoder
        lstm_hidden=128,
        lstm_layers=1,
        bidirectional=True,
        use_last_state=False  # if True, use last hidden state(s); else mean-pool over all outputs
    ):
        super().__init__()
        ...
        # We feed a sequence of length T_reduced, where each step is latent_dim features.
        self.lstm = nn.LSTM(
        ...
        # Two small MLP heads for the two tasks
        ...
        self.decoder = nn.Sequential(
            ...
            nn.Tanh()  # adjust depending on data scaling
        )
    ...
    def classify_from_z(self, z):
        ...
        # out_seq: (B, T_reduced, lstm_hidden * num_directions)
        # hn: (num_layers * num_directions, B, lstm_hidden)
        out_seq, (hn, cn) = self.lstm(seq)

        if self.use_last_state:
            # Concatenate the last forward/backward states from the top LSTM layer
    ...
    def forward(self, x, classify=False):
        """
        x: (B, C_in=24, L)
        If classify=False: returns reconstruction with length trimmed to input.
        ...
        if classify:
            ...
            # In case of any off-by-one from striding, trim to input length
